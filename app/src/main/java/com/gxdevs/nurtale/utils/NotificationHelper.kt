...
class DailyReminderReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        val customNotif = try {
            runBlocking {
                val db = com.gxdevs.nurtale.data.AppDatabase.getDatabase(context)
                val recentEntries = db.journalDao().getAllEntriesSync().takeLast(7)
                com.gxdevs.nurtale.utils.TailoredWellbeingEngine.generateTailoredNotification(recentEntries)
            }
        } catch (_: Exception) {
            val variant = reminderVariants.random()
            com.gxdevs.nurtale.utils.TailoredWellbeingEngine.CustomNotificationVariant(variant.title, variant.text, "Sage")
        }

        val notif = NotificationCompat.Builder(context, CHANNEL_DAILY)
            .setSmallIcon(com.gxdevs.nurtale.R.drawable.scroll)
