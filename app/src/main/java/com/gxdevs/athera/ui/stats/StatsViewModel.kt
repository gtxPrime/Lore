    ...
    private fun calculateStats(entries: List<JournalEntry>): StatsState {
        ...
        for (entry in entries) {
            ...
            if (!entry.emotions.isNullOrBlank()) {
                try {
                    val ems = com.google.gson.Gson().fromJson(entry.emotions, Array<com.gxdevs.athera.ui.journal.Emotion>::class.java)
                    ems.forEach { em ->
            ...
            for (i in recent.indices.reversed()) {
                try {
                    if (!recent[i].emotions.isNullOrBlank()) {
                        val ems = com.google.gson.Gson().fromJson(recent[i].emotions, Array<com.gxdevs.athera.ui.journal.Emotion>::class.java)
                        if (ems.isNotEmpty()) {
