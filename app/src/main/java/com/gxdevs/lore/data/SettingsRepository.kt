...
class SettingsRepository(private val context: Context) {
    ...
    companion object {
        ...
        // Pet nicknames: one key per petId — "pet_nickname_{petId}"
        fun petNicknameKey(petId: String) = stringPreferencesKey("pet_nickname_$petId")
    }
    ...
    /** Returns all pet nicknames as a map of petId -> nickname. */
    val allPetNicknames: Flow<Map<String, String>> = context.dataStore.data.map { prefs ->
        prefs.asMap().entries.mapNotNull { (key, valObj) ->
            if (key.name.startsWith("pet_nickname_")) {
                val petId = key.name.removePrefix("pet_nickname_")
                val nickname = valObj as? String ?: return@mapNotNull null
                petId to nickname
            } else null
        }.toMap()
    }

    /** Returns the user-set nickname for a pet, or null if no nickname has been set. */
    fun getPetNickname(petId: String): Flow<String?> = context.dataStore.data
        .map { it[petNicknameKey(petId)] }

    /** Saves or clears a pet nickname. Pass null to remove the nickname and revert to the JSON name. */
    suspend fun setPetNickname(petId: String, nickname: String?) {
        context.dataStore.edit { prefs ->
            if (nickname.isNullOrBlank()) prefs.remove(petNicknameKey(petId))
            else prefs[petNicknameKey(petId)] = nickname.trim()
        }
    }
}
